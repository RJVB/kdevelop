...
void loadTopDUContextData(const uint topContextIndex, LoadType loadType, F callback)
{
    ...
    uint readValue;
    store.read(reinterpret_cast<char*>(&readValue), sizeof(uint));
...
void TopDUContextDynamicData::DUChainItemStorage<Item>::loadData(TopDUContextStore* store) const
{
    ...
    store->read(reinterpret_cast<char*>(offsets.data()), sizeof(ItemDataInfo) * offsets.size());
...
void TopDUContextDynamicData::DUChainItemStorage<Item>::writeData(TopDUContextStore* store)
{
    ...
    store->write(reinterpret_cast<const char*>(&writeValue), sizeof(uint));
    store->write(reinterpret_cast<const char*>(offsets.data()), sizeof(ItemDataInfo) * offsets.size());
}
...
void TopDUContextDynamicData::loadData() const
{
    ...
    auto* store = new TopDUContextStore(m_topContext->ownIndex());
    ...
    if (m_mappedData) {
        ...
        store->close(); //Close the store, so there are less open file descriptors(May be problematic)
    } else {
